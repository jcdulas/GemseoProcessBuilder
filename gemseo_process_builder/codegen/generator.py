...
GENERATED_BY = "Generated by GEMSEO Process Builder"
"""Starts the paragraph of the docstring telling where a script comes from."""
...
def generate(
    ...
    description = ""
    if own_file:
        # The script is the project: its docstring gives its name and
        # description back when it is opened again.
        paragraphs = project.metadata.description.strip().split("\n\n")
        description = "".join(
            textwrap.fill(" ".join(paragraph.split()), LINE_LENGTH) + "\n\n"
            for paragraph in paragraphs
            if paragraph.strip()
        )
    elif target.id != project.root.id:
        title += f": {target.name}"
    source_note = f" from {project_file}" if project_file and not own_file else ""
    origin = textwrap.fill(
        f"{GENERATED_BY} {__version__}{source_note} "
