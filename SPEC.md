...
tests/
  python/                  # pytest
  js/                      # node --test
examples/                  # example GEMSEO scripts (Sellar, SSBJ, …)
```
...
A project is saved as a GEMSEO script (§ 4.2.2). The project format below is the format of older versions, which still open (they are saved as scripts), of the autosave, and of the snapshot of the project kept in each run folder:
...
- **Reading**: the text of a script cannot be read reliably, so the worker runs it, as `python script.py` would, and records how the study is built:
  ...
  - `set_algorithm` of the nested scenarios: their algorithms and settings;
  - `from_pickle`: the file each loaded model comes from;
  ...
  - A script without scenario stops at the first discipline it executes: an MDA or a chain becomes the process of the project. So does a discipline running a nested scenario (a chain holding an optimization as one of its steps).
- **Conversion**:
  ...
  - a `SurrogateDiscipline` of a model loaded with `from_pickle` becomes a surrogate component: with the metadata of the application next to the file, as the wizard saved it; without, the model is described by its type, and its ports are read by loading it in the worker;
  - a `RemappingDiscipline` becomes the component it renames the variables of, the new names as the global names of its ports; variables named `<component>:<variable>` make it an isolated instance (§ 5), and another component reading `<component>:<variable>` is linked to it;
  - a `SurrogateDiscipline` whose model comes from a file with its metadata (§ 12) becomes a surrogate component;
  ...
  - The fast mode (§ 11) is on when the script turned the checks of GEMSEO off; `n_processes` of a DOE becomes its parallel evaluations.
  - What cannot be kept is listed to the user.
- **Studies in several files**: the modules of the folder of the script it imports are read again at each reading (forgotten by the worker before), so that their changes are seen; those of a Python installed in the folder are kept.
- **Name and description**: from the docstring of the script. When it says it was written by the application (a paragraph starting with `Generated by GEMSEO Process Builder`), its first line is the name of the project and the paragraphs before that one its description; otherwise the name is the file name and the docstring the description.
- **Ids**: made of the names from the root (`n-Optimizer.Sellar1`), so that reading the same script twice gives the same ids.
...
*Save* writes a project as a GEMSEO script, `<name>.py`, and nothing else (`core/script_project.py`).
...
- **The script** is the one the code generation writes (§ 10) for the whole project:
  ...
  - Its docstring starts with the name and the description of the project.
  - The functions and classes of components defined in the script itself are used directly, not imported. Files in the folder of the script are found from it (`Path(__file__).parent / "solver.gpbwrap.json"`), so that the folder can move.
- **The code of the user is kept**:
  - The application rewrites only its functions (`build_disciplines`, `build_design_space`, `build_samples`, `build_scenario`, `execute_scenario`, `build_process`, `main`), its imports, its path constants (`*_FOLDER`, `*_WRAPPER`, `*_MODEL`) and the `if __name__ == "__main__"` block.
...
All drivers are **containers**: the workflow they drive is made of their children, as in ModelCenter and GEMSEO. On the canvas, a driver behaves like an assembly (§ 8.2): a card at its level, whose children are shown by entering it or expanding it in place.
...
- Assemblies and drivers (optimization, DOE, parametric study, MDA) are containers with the icon and color of their type: a card at their level, whose children are not shown there. Double-click to enter one (the breadcrumb goes back up), or expand it in place as a tinted frame around its children. Double-clicking the name of a driver opens its editor.
...
- Dragging from a driver onto a node (or from a node onto a driver) puts the node under the control of the driver: it moves into the driver.
- **Execution arrows**:
  ...
  - In an assembly run automatically, the handle shows when the pointer is over a node. Drawing an arrow makes the assembly a chain, starting from the order of the dependencies, as one undo step. A driver is a node like the others: an optimization becomes a step of a sequence (pre-processing → optimization → post-processing), run through an `MDOScenarioAdapter` with the interface of the driver.
...
```python
"""Sellar MDF: Optimizer.

Generated by GEMSEO Process Builder 0.1.0 from sellar_mdf.gpb.json on 2026-01-01.
Run it with: python sellar_mdf_optimizer.py
"""
...
This is the golden script of the Sellar MDF example, checked against GEMSEO 6.3: `add_constraint` must always be given `constraint_type`, since its default is `"eq"`. Two more rules come from GEMSEO 6.3:

- values typed in the diagram are set with `discipline.default_input_data.update({...})` on every discipline using the variable;
- a `RemappingDiscipline` renaming inputs gets `set_jacobian_approximation()`, because GEMSEO 6.3 cannot differentiate it exactly.
...
Their projects are stored in `tests/python/fixtures/examples/` and run by `tools/smoke_installed.py`, compared with the expected results. `tools/build_examples.py` writes them as the GEMSEO scripts of `examples/` (a test checks that the scripts are up to date); the surrogate case has no script, its surrogate being built in the application:
...
- `examples/wingBiLevel100k/`: the same wing in bi-level at scale, 100,000 local variables (50,000 twists, 50,000 skin thicknesses), on exact gradients. The sub-optimizations are adapters (`MDOScenarioAdapter`) whose only output is their optimum: the post-optimal analysis gives its derivatives from the Lagrange multipliers of their active sets, so that the system level optimizes with gradients (SLSQP) and converges in about 15 iterations, in about 30 s.
